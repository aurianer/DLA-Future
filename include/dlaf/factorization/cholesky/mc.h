...
void Cholesky<Backend::MC, Device::CPU, T>::call_L(comm::CommunicatorGrid grid,
  ...
  for (SizeType k = 0; k < nrtile; ++k) {
    ...
    if (kk_rank.col() == this_rank.col()) {
      if (kk_rank.row() == this_rank.row()) {
        ...
        if (kk_idx.row() < nrtile - 1)
          comm::send_tile(executor_mpi, mpi_task_chain, Coord::Col, panel_col_t.read(kk_offset));
      }
      else {
        if (kk_idx.row() < nrtile - 1)
          comm::recv_tile<T>(executor_mpi, mpi_task_chain, Coord::Col, panel_col_t(kk_offset),
                           kk_rank.row());
      }
    }
    ...
    for (SizeType i = distr.nextLocalTileFromGlobalTile<Coord::Row>(k + 1);
      ...
      if (kk_rank.col() == this_rank.col()) {
        trsm_panel_tile(executor_hp, panel_col_t.read(kk_offset), mat_a(ik_idx));
